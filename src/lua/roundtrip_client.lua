...
function calc_roundtrip(gh)
   if gh.state == gh.SUCCEEDED then
      ...
      local rcvd = roslua.Time:new(gh.result.values.result.values.received[1],
				   gh.result.values.result.values.received[2])
	 ...
	 if run >= num_runs then
	    ...
	    goal = acl.actspec.goal_spec:instantiate()
	    goal.values.start = roslua.Time.now()
...
local goal_id = acl:send_goal(goal, calc_roundtrip)
